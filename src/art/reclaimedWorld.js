...
import { rndmRng, coinflip, groupBy } from '../utils/helpers'
...
function drawTree(h) {
  ...
  if (Math.random() > 0.66)
    ctx.arc(
      h.x,
      trunkY - leavesR,
      Math.round(leavesR / rndmRng(2, 1.1)),
      0,
      2 * Math.PI,
    )
  ctx.fill()
...
function drawPoles(t) {
  ...
  ctx.quadraticCurveTo(
    Math.round(start.x + (endX - start.x) / 2 + 35),
    cpy,
    endX,
    endY,
  )
...
function drawCables(rows, t) {
  let start = {
    x: rndmRng(0, -100),
    y: Math.round(h * rndmRng(0.2 + t * 0.11, -0.05 + t * 0.06)),
  }
  ...
  const endY = Math.round(h * rndmRng(0.2 + t * 0.11, -0.05 + t * 0.06))
  const endX = rndmRng(w + 100, w)
...
function drawCables2(amt) {
  ...
  for (let i = amt; i--; ) {
    ...
    const p1 = { x: rndmRng(w * 0.7, w * 0.3), y: h - rndmRng(250, 0) }
...
function generateHillRow(type) {
  ...
  while (x < w + 50) {
    const decrease = Math.abs(w / 2 - x) * 0.35
    const r = rndmRng(300, 100)
    const y = Math.round(
      h * rndmRng(0.7 + type * 0.06, 0.5 + type * 0.06) - decrease + r,
    )
    const tree = Math.random() > 0.75 ? true : false
...
function generateGround(type) {
  ...
  while (x < w + 50) {
    ...
    const y = Math.round(h * rndmRng(0.99, 0.88) + r)
...
function generateMonolith(type, x) {
  ...
  const y = Math.round(h * rndmRng(0.2 + type * 0.11, -0.05 + type * 0.06))
...
function generateMonoRow(t) {
  let x = Math.round(w * rndmRng(0.16, 0.02))
  while (x < w - 100) {
...
function drawGradient(type) {
  ...
  type === 0
    ? gradient.addColorStop(0.05, `hsla(${colorsG[type]}, 0)`)
    : gradient.addColorStop(
        (2 * (3.5 / (2 - type * 0.13))) / 8 - 0.1,
        `hsla(${colorsG[type]}, 0)`,
      )

  gradient.addColorStop(
    0.4 + (type * 0.36) / ((type + 1) * 0.53),
    `hsla(${colorsG[type]}, 1)`,
  )
...
function drawDudes() {
  let peaks = [
    {
      x: Math.round(w * rndmRng(0.5, 0.3)),
      y: h - rndmRng(270, 200),
      r: rndmRng(30, 20),
    },
  ]
  ...
  peaks.push({
    x: Math.round(peaks[0].x + w * rndmRng(0.2, 0.1)),
    y: h - rndmRng(270, 200),
    r: rndmRng(30, 20),
  })
  ...
  peaks.forEach((p, i) => {
    ...
    var grdRadial = ctx.createRadialGradient(
      p.x,
      p.y + p.r,
      p.r,
      p.x + rndmRng(p.r, p.r * -1),
      p.y + rndmRng(p.r * 4, p.r * -2),
      p.r + rndmRng(10, -10),
    )
    grdRadial.addColorStop(0, `hsla(${baseHue + 14}, 90%, 12%, 1)`)
    grdRadial.addColorStop(0.8, `hsla(${baseHue + 44}, 23%, 3%, 1)`)
    ...
    if (i === 1) {
      ...
      let angle =
        (Math.atan2(ground.y - flag.y, ground.x - flag.x) * 180) / Math.PI
      let l = rndmRng(80, 40)
      ctx.beginPath()
      ...
      ctx.fillStyle = `hsla(${baseHue + 64}, 9%, 2%, 1)`
      ctx.lineTo(
        flag.x - l * Math.cos((angle * Math.PI) / 180),
        flag.y - l * Math.sin((angle * Math.PI) / 180),
      )
      ctx.lineTo(
        flag.x - l * Math.cos(((60 + angle) * Math.PI) / 180),
        flag.y - l * Math.sin(((60 + angle) * Math.PI) / 180),
      )
      ctx.lineTo(flag.x, flag.y)
      ctx.fill()
      ctx.stroke()
...
export function loadWorld() {
  ...
  ground = []
  sun = {
    x: Math.round(w * rndmRng(0.7, 0.3)),
    y: Math.round(h * rndmRng(0.45, -0.05)),
  }
