...
function drawCircle(m) {
  ...
  //(m.x*.9)(m.x-m.r/2)*1.1
  //100 //50
  let xTopLeft = Math.round(rndmRng(m.x * 0.9, m.x - m.r / 2));
  let yTopLeft = Math.round(rndmRng(m.y * 0.9, (m.y - m.r / 2) * 1.1));
  let rTopLeft = Math.round(rndmRng(m.r / 12, m.r / 24));
  console.log(`m.x: ${m.x} | m.y: ${m.y} | ${m.r} m.r`);
  console.log(
    `xTopLeft: ${xTopLeft} | yTopLeft: ${yTopLeft} | ${rTopLeft} rTopLeft`
  );
...
function createCircles(n) {
  for (let i = n; n--; ) {
    ...
    let r = Math.round(rndmRng(h * 0.19, h * 0.09));
    circles.push({ x, y, r });
  }
}
...
export function loadOracular() {
  ...
  createCircles(Math.round(rndmRng(avgSize * 0.33, avgSize * 0.15)));
