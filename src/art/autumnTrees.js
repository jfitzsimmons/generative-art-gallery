...
function buildBranch(start, layer) {
  ...
  for (let i = 2; i--; ) {
    let newBranchH = rndmRng(branchH * 1.2, branchH * 0.8);
    let newX = Math.round(
...
function drawBranch(b, i, s) {
  const found = treePoints.find(
    (e) => Math.abs(e.tc.x - b.bc.x) === b.topW / 4
  );
