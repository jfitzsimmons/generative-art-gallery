...
function backgroundGradients(layers) {
  for (let i = layers; i--; ) {
    let outR = rndmRng(w * 0.4, w * 0.1)
    let outX = rndmRng(w + w * 0.1, 0 - w * 0.1)
    let outY = rndmRng(h + h * 0.1, 0 - h * 0.1)
    let inR = rndmRng(outR * 0.3, outR * 0.01)
...
function bursts(bursts) {
  for (let b = 0; b < bursts; b++) {
    ...
    let x = rndmRng(w, 0)
    let y = rndmRng(h, 0)
    let size = Math.round(rndmRng(h * 0.29, h * 0.08))
    let g = 0
    setDashedLines()
...
function mainCircle() {
  let x = 0
  let y = Math.round(rndmRng(h * 0.66, h * 0.33))
