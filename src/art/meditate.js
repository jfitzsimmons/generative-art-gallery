...
function points(x, y, r) {
  ...
  for (let i = 0; i < lines; i++) {
    ...
    p1.x < w * 0.5
      ? curvedLine(x1, y1, x2, true)
      : curvedLine(x2, y1, x1, true);
    ctx.beginPath();
    ctx.moveTo(x1, y1);
    ctx.lineTo(x2, y2);
    ctx.stroke();
...
function drips() {
  let count = 0;
  let x = rndmRng(30, -30);
  let step = Math.round(rndmRng(w * 0.06, w * 0.03));
...
function drawMeteor(m) {
  ...
  for (let j = 1.5; j > 0.5; j -= 0.05) {
    ...
    let endX = Math.round(m.r * Math.cos(Math.PI * (j + reach * 0.05)) + m.x);
...
function createFields(x, stalks, mod) {
  ...
  let maxHeight = rndmRng(h * 0.5, h * 0.36);
  let minHeight = rndmRng(h * 0.35, h * 0.21);
  let oldHeight = 0;
...
function createStripes() {
  let cols = Math.round(rndmRng(w * 0.4, w * 0.2) / 80);
  let rows = Math.round(rndmRng(h * 0.4, h * 0.2) / 60);
  let x = p2.x;
  let y = Math.round(p2.y + h * 0.05);
...
function createMeteors(n) {
  for (let i = n; n--; ) {
    let x = rndmRng(w, 0);
    let y = Math.round(rndmRng(h * 0.5, 0));
    let r = Math.round(rndmRng(h * 0.16, h * 0.07));
    meteors.push({ x, y, r });
  }
}

function generateNoise() {
  let squares = Math.round(Math.round((h * w) / (h * 0.09 * (w * 0.09))));
  let x,
    y = 0;
  ...
  ctx.fillStyle =
    sunColors.sun === "#DA2E20"
      ? `rgba(${number}, ${number - 25},${number - 50}, 0.${Math.round(
          rndmRng(2, 1)
        )}`
      : `rgba(${number - 50}, ${number - 25},${number}, 0.${Math.round(
          rndmRng(2, 1)
        )}`;
...
export function loadMeditations() {
  ...
  let size = Math.round(rndmRng(h * 0.34, h * 0.19));
  ctx.beginPath();
  ctx.arc(
